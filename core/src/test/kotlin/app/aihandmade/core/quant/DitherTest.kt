 ...
 * anything here.
 *
 * ditherFloydSteinberg reads the image geometry from the OkLabPlanes itself, walks pixels in raster
 * order, adds the diffused error to each pixel's OkLab, picks the nearest palette colour by
 * OKLab-Euclidean (deltaSqOk, ties -> smallest index), writes that index, and diffuses the residual
 * OkLab error to the right (7/16), down-left (3/16), down (5/16) and down-right (1/16). Pure OKLab
 * working space — no CIEDE2000, no Lab conversion.
 *
...
class DitherTest {
    ...
    private fun planes(L: FloatArray, width: Int, height: Int) =
        OkLabPlanes(L, FloatArray(L.size) { 0f }, FloatArray(L.size) { 0f }, width, height)
    ...
    fun exactPaletteMatchDoesNotDither() {
        ...
        val out = ditherFloydSteinberg(img, blackWhite)
    ...
    fun deterministic() {
        ...
        assertArrayEquals(ditherFloydSteinberg(img, blackWhite), ditherFloydSteinberg(img, blackWhite))
    }
    ...
    fun invalidInputsThrow() {
        ...
        assertThrows(IllegalArgumentException::class.java) { ditherFloydSteinberg(img, emptyP) }
    }
}
